...
util.url = 'http://47.99.35.178/api/v1'

util.urlssh = 'http://47.99.35.178'

util.auth = 'http://47.99.35.178/api/v1/login/'

util.wflowssh = 'http://47.99.35.178/workflow/v2'

util.webssh = 'ws://47.99.35.178/ssh/ws?id='

util.stage = 'http://47.99.35.178/stage/v3'

util.applyserver = 'http://47.99.35.178/order/v4'
...
util.taglist = function (vm, name) {
  ...
  if (name === 'myorder') {
    vm.$store.state.pageOpenedList.push({'title': '项目管理', 'name': 'myorder'})
  }
  if (name === 'usergroup') {
    vm.$store.state.pageOpenedList.push({'title': '用户管理', 'name': 'usergroup'})
  }
  if (name === 'gameManage') {
    vm.$store.state.pageOpenedList.push({'title': '业务管理', 'name': 'gameManage'})
  }
  if (name === 'hostManage') {
    vm.$store.state.pageOpenedList.push({'title': '主机管理', 'name': 'hostManage'})
  }
